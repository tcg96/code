size(900, 300); // you can change it to what you want, but if you don't use a 3/1 aspect ratio, it'll look more screwed up than the actual logo
float s = width/3, t = s*2;
float stickx = s/4, sticky = height-stickx*2; // x and y coordinates for the first stick
float stickwidth = s/10.5;
float circlex = s/1.75, circley = s/2.33;
float diameter = height/2.8;
...
// I know about the setup and draw methods, but this sketch is so simple you don't need to use them