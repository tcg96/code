...
class FakeItOO { // an object-oriented version of FakeIt. This one uses an actual "Flower" instead of a String!
    public static void main (String[] args) {
        ...
        Object[] thisArray = new Object[1];       // create a new array called "thisArray" with only one position (pointless, but doable). it's an Object array instead of a Flower array, because who says thisArray only has flowers? remember, the actual lyric is "disarray"
        thisArray[0] = flowerIn;                  // take the flower and put it in the array at index 0 (the only array position there is in this case)
        Object flowerOut = thisArray[0];          // then get it out of the array. what you've done: "flower" through thisArray
        System.out.println(flowerOut.toString()); // this line is optional, but I added it so there's written proof that the Flower has been through its journey in and out of thisArray
    }
}
